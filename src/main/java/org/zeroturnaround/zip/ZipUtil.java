...
public final class ZipUtil {
     ...
     * @param zip     ZIP file.
     * @param name    entry name.
     * @param charset charset to be used to process the zip
     * @return contents of the entry or <code>null</code> if it was not found.
     */
     ...
     * @param name    entry name.
     * @param file    target file to be created or overwritten.
     * @param charset charset to be used processing the zip
     * @return <code>true</code> if the entry was found and unpacked,
     * <code>false</code> if the entry was not found.
     */
     ...
     * @param zip     input ZIP file.
     * @param action  action to be called for each entry.
     * @param charset Charset used to processed the ZipFile with
     * @see ZipEntryCallback
     * @see #iterate(File, ZipInfoCallback)
     */
        ...
        try {
            ...
            while (en.hasMoreElements()) {
                ...
                try {
                    ...
                    throw new org.zeroturnaround.zip.ZipException("Failed to process zip entry '" + e.getName() + "' with action " + action, ze);
     ...
     * @param entryNames names of entries to iterate
     * @param action     action to be called for each entry.
     * @param charset    charset used to process the zip file
     * @see ZipEntryCallback
     * @see #iterate(File, String[], ZipInfoCallback)
     */
     ...
     * then throw a ZipBreakException.
     *
     * @param is      input ZIP stream (it will not be closed automatically).
     * @param action  action to be called for each entry.
     * @param charset charset to process entries in
     * @see ZipEntryCallback
     * @see #iterate(File, ZipEntryCallback)
     */
            ...
            try {
                ...
                while ((entry = in.getNextEntry()) != null) {
                    try {
                        ...
                        throw new org.zeroturnaround.zip.ZipException("Failed to process zip entry '" + entry.getName() + " with action " + action, ze);
                    } catch (ZipBreakException ex) {
                        break;
                    }
                }
     ...
     * @param zip       input ZIP file.
     * @param outputDir output directory (created automatically if not found).
     * @param charset   charset used to unpack the zip file
     */
     ...
     * The output directory must not be a file.
     *
     * @param is        inputstream for ZIP file.
     * @param outputDir output directory (created automatically if not found).
     */
     ...
     * @param is        inputstream for ZIP file.
     * @param outputDir output directory (created automatically if not found).
     * @param charset   charset used to process the zip stream
     */
     ...
     * @param outputDir output directory (created automatically if not found).
     * @param mapper    call-back for renaming the entries.
     * @param charset   charset to use when unpacking the stream
     */
    ...
    private static class Unwrapper implements ZipEntryCallback {
        ...
        public void process(InputStream in, ZipEntry zipEntry) throws IOException {
            ...
            if (rootDir == null) {
                ...
                throw new org.zeroturnaround.zip.ZipException("Unwrapping with multiple roots is not supported, roots: " + rootDir + ", " + root);
            }
        ...
        private String getRootName(final String name) {
            ...
            if (idx < 0) {
                throw new org.zeroturnaround.zip.ZipException("Entry " + newName + " from the root of the zip is not supported");
            }
     ...
     * @param preserveRoot  true if the resulted archive should have the top directory entry
     */
     ...
     * @param fileToPack  file that needs to be zipped.
     * @param destZipFile ZIP file that will be created or overwritten.
     * @param fileName    the name for the file inside the archive
     */
     ...
     * @param filesToPack      files that needs to be zipped.
     * @param destZipFile      ZIP file that will be created or overwritten.
     * @param compressionLevel ZIP file compression level (speed versus filesize), e.g. <code>Deflater.NO_COMPRESSION</code>, <code>Deflater.BEST_SPEED</code>, or
    ...
    public static void pack(File sourceDir, File targetZip, NameMapper mapper, int compressionLevel) {
        if (!sourceDir.exists()) {
            throw new org.zeroturnaround.zip.ZipException("Given file '" + sourceDir + "' doesn't exist!");
        }
        ZipOutputStream out = null;
        try {
     ...
     *
     * @param sourceDir root directory.
     * @param os        output stream (will be buffered in this method).
     * @since 1.10
     */
    ...
    private static void pack(File dir, ZipOutputStream out, NameMapper mapper, String pathPrefix, boolean mustHaveChildren) throws IOException {
        ...
        if (filenames == null) {
            if (!dir.exists()) {
                throw new org.zeroturnaround.zip.ZipException("Given file '" + dir + "' doesn't exist!");
            }
     ...
     * @param bytes             new entry bytes (or <code>null</code> if directory).
     * @param destZip           new ZIP file created.
     * @param compressionMethod the new compression method (<code>ZipEntry.STORED</code> or <code>ZipEntry.DEFLATED</code>).
     */
     ...
     * @param zip     an existing ZIP file (only read).
     * @param entries new ZIP entries appended.
     * @param destOut new ZIP destination output stream
     */
     ...
     * Copies an existing ZIP file and appends it with new entries.
     *
     * @param is      an existing ZIP input stream.
     * @param entries new ZIP entries appended.
     * @param destOut new ZIP destination output stream
     * @since 1.9
     */
     ...
     *
     * @param is  source stream (contains ZIP file).
     * @param out target ZIP stream.
     */
     ...
     * @param zip         an existing ZIP file (only read).
     * @param path        new ZIP entry path.
     * @param transformer transformer for the given ZIP entry.
     * @param destZip     new ZIP file created.
     * @return <code>true</code> if the entry was replaced.
     ...
     *
     * @param zip     an existing ZIP file (only read).
     * @param entry   transformer for a ZIP entry.
     * @param destZip new ZIP file created.
     * @return <code>true</code> if the entry was replaced.
     */
     ...
     *
     * @param zip     an existing ZIP file (only read).
     * @param entries ZIP entry transformers.
     * @param destZip new ZIP file created.
     * @return <code>true</code> if at least one entry was replaced.
     */
     ...
     * </ol>
     *
     * @param path name of the entries.
     * @param e1   first entry (required).
     * @param e2   second entry (may be <code>null</code>).
